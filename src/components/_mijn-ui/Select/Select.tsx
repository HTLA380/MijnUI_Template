...
const Select = ({ open, onToggle, onSelect, children, defaultSelectedIndex, placement = "bottom" }: SelectProps) => {
  // State management
  const [isOpen, setIsOpen] = useControlledState(open, false, onToggle);
  const [activeIndex, setActiveIndex] = React.useState<number | null>(null);
  const [selectedIndex, setSelectedIndex] = React.useState<number | null>(defaultSelectedIndex ?? null);
...
type SelectTriggerProps = {
  children?: React.ReactNode | ((props: { selectedLabel: string | null }) => React.ReactNode);
...
const SelectTrigger = polymorphicForwardRef<"button", SelectTriggerProps>(
  ({ renderAs: Component = "button", children, className, ...props }, ref) => {
    const { refs, getReferenceProps, selectedLabel } = React.useContext(SelectContext);
    ...
    return (
      <Component
        ...
        {...props}
      >
        {typeof children === "function" ? children({ selectedLabel }) : children}
      </Component>
    );
  }
);
...
const SelectContent = React.forwardRef<HTMLDivElement, React.ComponentProps<"div">>(
  ({ className, children, ...props }, ref) => {
    const { context, elementsRef, labelsRef, refs, floatingStyles, getFloatingProps } = React.useContext(SelectContext);
...
const SelectOption = polymorphicForwardRef<"button", SelectOptionProps>(
  ({ renderAs: Component = "button", className, value, children, ...props }, ref) => {
    const { activeIndex, selectedIndex, getItemProps, handleSelect, onSelect } = React.useContext(SelectContext);
