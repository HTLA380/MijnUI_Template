...
const useDropdownMenuContext = () => {
  ...
  if (!context) {
    throw new Error("DropdownMenu components must be rendered within the Menu component");
  }
  return context;
};
...
const DropdownMenuInner = ({ open, onToggle, placement, children }: DropdownMenuProps) => {
...
const DropdownMenuTrigger = polymorphicForwardRef<"button", React.ComponentProps<"button">>(
  ({ renderAs: Component = "button", className, children, ...props }, forwardedRef) => {
    const item = useListItem();

    const { activeIndex, getItemProps, setHasFocusInside, refs, isNested, isOpen, hasFocusInside, getReferenceProps } =
...
export const DropdownMenuContent = React.forwardRef<HTMLDivElement, React.ComponentProps<"div">>(
  ({ className, children, style, ...props }, forwardedRef) => {
    const { refs, isNested, context, getFloatingProps, floatingStyles, labelsRef, elementsRef } =
      useDropdownMenuContext();
      ...
      <FloatingList labelsRef={labelsRef} elementsRef={elementsRef}>
        {isMounted && (
          <FloatingPortal>
            ...
            >
              <div ref={refs.setFloating} style={floatingStyles} className="outline-none" {...getFloatingProps()}>
                <div
                  className={cn(
...
export const DropdownMenuItem = polymorphicForwardRef<"button", React.ComponentProps<"button"> & { label: string }>(
  ({ renderAs: Component = "button", className, disabled, label, children, ...props }, forwardedRef) => {
