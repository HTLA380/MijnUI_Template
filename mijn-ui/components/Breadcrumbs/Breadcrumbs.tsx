...
const Breadcrumbs = React.forwardRef<HTMLElement, React.ComponentProps<"nav">>(
  ({ className, ...props }, ref) => (
    <nav
      ref={ref}
      className={cn(
        "flex flex-wrap items-center justify-start",
        "gap-1 text-muted-text",
        className,
      )}
  ...
  ({ renderAs: Component = "a", className, ...props }, ref) => {
    return (
      <Component
        ref={ref}
        className={cn(
          "flex items-center text-muted-text hover:text-main-text hover:underline",
          className,
        )}
        {...props}
      />
...
const BreadcrumbsSeparator = React.forwardRef<
  HTMLElement,
  React.ComponentProps<"span">
>(({ className, ...props }, ref) => (
