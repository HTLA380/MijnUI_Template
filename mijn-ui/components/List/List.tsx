...
import { ToggleProvider, useToggleContext } from "@/context";
...
const ListItemLeadingIcon = React.forwardRef<
  HTMLDivElement,
...
const ListDivider = React.forwardRef<HTMLHRElement, React.ComponentProps<"hr">>(
  ({ className, ...props }, ref) => (
    <hr
      ref={ref}
      className={cn("my-2 border-t border-main-border", className)}
      {...props}
    />
  ),
);
...
const ListSubMenuTrigger = polymorphicForwardRef<
  "button",
...
>(({ renderAs: Component = "button", children, className, ...props }, ref) => {
  ...
  return (
    <Component
      ...
      className={cn(
        "group/collapse inline-flex h-10 w-full items-center gap-2 rounded-md px-3 py-2 text-sm hover:bg-accent hover:text-accent-text data-[state=open]:bg-primary data-[state=open]:text-primary-text data-[state=open]:hover:bg-primary data-[state=open]:hover:text-primary-text",
        className,
      )}
      {...props}
    >
      ...
      <span
        className={cn("transition group-data-[state=open]/collapse:rotate-180")}
      >
...
const ListSubMenuContent = React.forwardRef<
  HTMLUListElement,
  ListSubMenuContentProps
>(({ children, animateOnMount, className, ...props }, ref) => {
  ...
  const { transitionStatus, isMounted } = useTransition(
    isOpen,
    300,
    !!animateOnMount,
  );
